...
async function checkAndNotify() {
  ...
  // Sadece saat 20:00 civarı (20-21 arası) ve bugün henüz gönderilmediyse
  const lastSent = await getStore('lastNotifDate');
  if (hour >= 20 && hour < 21 && lastSent !== today) {
    await setStore('lastNotifDate', today);
    const msg = REMINDER_MESSAGES[Math.floor(Math.random() * REMINDER_MESSAGES.length)];
